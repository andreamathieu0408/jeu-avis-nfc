...
const gains = [
  ...
  { label: "Un café offert ☕", color: "bg-blue-400" },
  { label: "Rien 😢", color: "bg-blue-100" },
  { label: "Livraison gratuite", color: "bg-blue-500" },
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-400 to-blue-600 flex flex-col items-center justify-center text-center px-4 font-sans">
      {step === "start" && (
        <div className="max-w-md">
          <h1 className="text-3xl md:text-4xl font-extrabold mb-4 text-white">🎁 Gagne un cadeau en laissant un avis !</h1>
          ...
          <button onClick={handleLeaveReview} className="bg-white text-blue-600 rounded-full px-6 py-3 font-bold text-lg shadow-lg hover:scale-105 transition">Laisser un avis ⭐</button>
        </div>
      )}

      {step === "wheel" && (
        <div className="flex flex-col items-center">
          <h1 className="text-3xl font-bold mb-6 text-white">🎉 Tourne la roue !</h1>
          <motion.div
          ...
          {!spinning && (
            <button onClick={handleSpin} className="mt-6 bg-white text-blue-600 rounded-full px-8 py-4 text-xl font-bold shadow-xl hover:rotate-6 transition">
              Lancer la roue
            </button>
          )}
        </div>
      ...
      {step === "result" && prize && (
        <div className="flex flex-col items-center">
          <h1 className="text-4xl font-bold text-white mb-4">🎊 Bravo !</h1>
          <div className={`text-white text-2xl mb-6 px-6 py-3 rounded-full ${prize.color}`}>Tu as gagné : <span className="font-bold">{prize.label}</span></div>
